...
export function UpdateRoleModal() {
  ...
  if (!selectedRoleForUpdate) return null

  return (
    <Dialog open={isUpdateRoleOpen} onOpenChange={handleClose}>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="update-role-name" className="text-sm font-medium">
                Name
              </Label>
              <Input
                id="update-role-name"
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="text-gray-500">{updateRoleName.length}/50</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {updateRolePermissions.map((group) => {
                    ...
                    <CollapsibleContent>
                      <div className="px-4 pb-4 space-y-3 border-t">
                        <div className="flex items-center space-x-2 pt-3">
                          <Checkbox
                            id={`update-select-all-${group.id}`}
                            checked={allChecked}
                            ref={(el:any) => {
                              if (el) el.indeterminate = someChecked && !allChecked
                            }}
                            onCheckedChange={() => toggleUpdateSelectAll(group.id)}
                          />
                          <Label
                            htmlFor={`update-select-all-${group.id}`}
                        ...
                        {group.permissions.map((permission) => (
                          <div key={permission.id} className="flex items-center space-x-2">
                            <Checkbox
                              ...
                              onCheckedChange={() => toggleUpdatePermission(group.id, permission.id)}
                            />
                            <Label htmlFor={`update-${permission.id}`} className="text-sm">
                              {permission.name}
                            </Label>
                          </div>
                        ))}
