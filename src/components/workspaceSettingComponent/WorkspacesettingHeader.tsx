...
import { updateWorkspaceSchema } from "@/Schemas/updateWorkspaceSchema" // Assuming this schema exists
...
function WorkspacesettingHeader() {
  ...
  const {
    ...
    () => getWorkspaceBySlug({ workspaceSlug: workspaceSlug as any}),
    true,
  )
  const {
    ...
    serverError,
  } = useRenameWorkspace({ workspaceSlug: workspaceSlug as string })
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const handleSave = (data: { name: any }) => {
